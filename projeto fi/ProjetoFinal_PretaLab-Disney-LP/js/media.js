...
const nota = (nota1 + nota2 + nota3 + nota4) / 4;
  if(nota === 0) 
  {mensagem = `Infelizmente você zerou a prova :(` } 
  else if (nota >= 0.1 && nota <=3) {mensagem= `Caramba, deu ruim, você obteve média ${nota}! Estude mais e tente novamente`}
  else if (nota >= 3.1 && nota <= 5.9) {mensagem= `Você obteve média de ${nota}`}
  else if (nota >= 6 && nota <= 7) {mensagem= `Você está na média com ${nota}`}
  else if (nota >= 7.1 && nota <= 9.9) {mensagem= `Notão sua média é ${nota}!}`}
  else if (nota >= 10) {mensagem= `Hoje é seu aniversário? Pq você ta de parabéns! 10 de média`}
  else if (nota1 < 0 || nota1 > 10 || nota2 < 0 || nota2 > 10 || nota3 < 0 || nota3 > 10 || nota4 < 0 || nota4 > 10) { mensagem = "Você não pode inserir uma nota menor que 0 ou maior que 10."}
